	...
	for (var i=0;i<repository.process.length;i++) {
		...
		try {
			exec(proc.exec, function (error, stdout, stderr) {
			  	...
			  	if (error) {
				  	jook.logger.error(lbl + ' error: ' + ansi_up.ansi_to_html(error), { repository : repository.name });
			  	}

			  	if (error || stderr) {
				  	jook.logger.error(lbl + ' std error: ' + ansi_up.ansi_to_html(stderr), { repository : repository.name });
				  	return;
			  	}
			});
		} catch (err) {
...
jook.getRepository = function(payLoad, callback) {
    if (payLoad == null || payLoad.repository == null || payLoad.repository.full_name == null || payLoad.ref == null) {
