...
jook.offLoad = function(payLoad, callback) {
	...
	for (var i=0;i<repository.process.length;i++) {
		...
		if (p == null) {
			jook.logger.info('Empty Process: ' + i, { repository : repository.name });
			continue;
		}

		jook.logger.info('Executing: ' + p.name + '[' + i + ']', { repository : repository.name });

		try {
			var pos = i;
			exec(p.exec, function (error, stdout, stderr) {
			  	if (error) {
			  		cbError('Error invoking exec: ' + error, { repository : repository.name });
			  		return;
			  	}

			  	if (stderr) {
			  		cbError('Error with exec: ' + stderr, { repository : repository.name });
			  		return;	
			  	}

			  	jook.logger.info('Process Out [' + pos + ']: ', { repository : repository.name });
			  	jook.logger.debug(stdout, { repository : repository.name });
			});
		} catch (err) {
